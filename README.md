...
### Frontend
This was supposed to be timeboxed to max 2hours, so I had to give up on custom frontend at all. 
As a quick workaround I have to propose the usage of 'fronted' generated by swagger. (http://localhost:8008/docs)
I'm using a single endpoint for both uploading a video and searching to eliminate scope creep introduced by following: 
1. the need for a persistent storage
2. modeling VideoFile domain
3. implementing frontend to coordinate requests to different endpoints
